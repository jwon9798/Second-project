    ...
    <header className="sticky top-0 z-50 border-b border-white/5 bg-[#07080f]/80 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <LanguageSwitcher />
          <Link href="/create" className="btn-primary hidden sm:flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white">
      ...
      {open && (
        <nav className="border-t border-white/5 bg-[#07080f]/95 px-4 py-4 md:hidden">
          {links.map((link) => (
            <Link key={link.href} href={link.href} onClick={() => setOpen(false)} className="block rounded-lg px-3 py-3 text-sm text-white/80">
              {link.label}
            </Link>
          ))}
          <Link href="/create" onClick={() => setOpen(false)} className="btn-primary mt-2 flex justify-center gap-2 rounded-xl py-3 text-sm font-semibold text-white">
            <Plus className="h-4 w-4" />
            {t("create")}
          </Link>
        </nav>
